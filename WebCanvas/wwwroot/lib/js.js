    ...
    if (window.innerWidth >= 1200) {
        ...
        $('#siteThemeBtn').on('click', function () { // ТОЖЕ САМОЕ ТОЛЬКО ЧТО БЫ МЕНЯЛСЯ ТЕКСТ
            ...
            if (Cookies.get('darkTheme') == 'false') {
                fastTip('#siteThemeBtn', '<br><h4>Тёмная тема</h4><span>(Меняет оформление сайта на тёмное)</span>');
                toggle.css('color', isToggleOn ? '#0060B4' : '#262626');
            }
            else {
                fastTip('#siteThemeBtn', '<br><h4>Светлая тема</h4><span>Меняет оформление сайта на светлое</span>');
...
function loadCanvas(id) { // КОДИНГ В РЕАЛЬНОМ ВРЕМЕИ
    ...
    function canvas() {
        ...
        $('#siteThemeBtn').on('click', function () {
            preview.open();
            var isDark = Cookies.get('darkTheme') == 'true' ? '<style> canvas { background: url("../images/setka3_black.png") } </style>' : '';
            preview.write(
                isCanvas ?
                    isDark +
                    "<canvas height=\"199px\" width=\"299\" id=\"canvas" + id + "\" style=\"margin:-8px;\"></canvas>" +
...
function fastTipNav(element, tool, text) {
    tool = $(tool);
    $(element).on('mouseenter', function () {
        tooltipText(text, tool, false, 0, -1);
        $(element).on('mouseleave', function () {
            tool.hide();
        });
    });
...
function setToolTip() {
    ...
    $('#menuItem1 a').on('mouseover', function () {
        if ($(this).is('[id]'))
            tooltipText('<br><strong>Вкладка:</strong> ' + linkList[$(this).attr('id') - 1], $('.tooltip.tip-egg'), true, 0, -1);
    });
    ...
    fastTip('#userBtn', '<br><h4>Профиль</h4><span>(Подробная информация о Вашем профиле)</span>');
    fastTip('#usersBtn', '<br><h4>Пользователи</h4><span>(Подробная информация о пользователях сайта)</span>');
...
function ReplyComment(ID, text) {
    ...
    var replyBtn = document.getElementById('resetReplyBtn');
    replyBtn.style.display = 'block';
}

function resetReply() {
    document.getElementById('isReplyInput').value = false;
    document.getElementById('replyIdInput').value = null;
